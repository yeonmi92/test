  ...
  <section class="blog-post-area section-margin">
 	<div class="container">
		...
		<c:if test="${sessionScope.memId!=null}">
			<a href="/together/member/myArticleList.git"><button type="button" class="btn btn-primary">글쓰기</button></a>
		</c:if>
		&nbsp;
		<div class="dropdown">
	      			...
	      			<c:forEach var="article" items="${articleList}">
		      			...
		      			<!-- 반복문 돌리기 -->
		      				...
		      				<td>
		      					...
		      					<a href="/together/board/content.git?board_num=${article.board_num}&pageNum=${pageNum}">
		      					${article.title}</a>
		      				</td>
		      				
		      				<td>
		      					<a onclick="infoView('${article.id}')">${article.id}</a>
		      				</td>
	                ...
	                <ul class="pagination pagination-sm" style="background-color: yellow;">
						<c:if test="${startPage>pageBlock}">
		                  <li class="page-item disabled">
		                    <a class="page-link" href="/together/member/myArticleList.git?pageNum=${startPage + pageBlock}">&laquo;</a>
		                  </li>
		                </c:if>
		                
		            	<c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
		                  	<li class="page-item active">
		                    	<a class="page-link pageNums" href="/together/member/myArticleList.git?pageNum=${i}&sel=${sel}">${i}</a>
		                  	</li>
						</c:forEach>
	                  <c:if test="${endPage<pageCount}">
		            	<li class="page-item">
		             		<a class="page-link" href="/together/member/myArticleList.git?pageNum=${startPage + pageBlock}">&raquo;</a>
		             	</li>
	                  </c:if>
	                </ul>
                </c:if>
